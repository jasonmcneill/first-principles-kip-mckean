...
if (!fs.existsSync(swPath)) fail('public/sw.js not found; run `npm run build` first');
...
const match = allMatches ? allMatches[allMatches.length - 1].match(/const LANG_SLUGS\s*=\s*(\{[\s\S]*?\});/) : null;
