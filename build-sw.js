...
(async () => {
  try {
    ...
    const langDirs = fs.readdirSync(i18nPath, { withFileTypes: true })
    ...
    if (!swSrc.includes(placeholder)) {
      throw new Error(`Placeholder ${placeholder} not found in ${swSrcPath}. Aborting build.`);
    }
  // Replace only the const assignment with the LANG_SLUGS object
  const finalSw = swSrc.replace('const LANG_SLUGS = {};', `const LANG_SLUGS = ${JSON.stringify(LANG_SLUGS, null, 2)};`);
